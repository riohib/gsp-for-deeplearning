...
from data_prep import dataloaders
...
parser.add_argument('--dataset', default='imagenet_dali',
                    choices=['imagenet_pytorch', 'imagenet_dali', 'tiny_imagenet'],
                    type=str, metavar='PATH', help='type of dataset to choose')
...
parser.add_argument("--lr-drop",  nargs="*",  type=int, default=[30, 60, 90],)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    num_classes = 200 if(args.dataset == 'tiny_imagenet') else 1000
    if args.pretrained:
        ...
        model = models.__dict__[args.arch](pretrained=False, num_classes=num_classes)
        flogger.info(f"Created model from Pytorch Model Zoo! LinearLayer: {model.fc}")
    ...
    # Get the Dataloaders
    train_loader, train_loader_len, val_loader, val_loader_len = dataloaders(args)
    ...
    scheduler = MultiStepLR(optimizer, milestones=args.lr_drop, gamma=0.1)
    for epoch in range(args.start_epoch, args.epochs):
        t0 = time.time() #timing        
        
        acc1, loss = train(train_loader, train_loader_len, model_gsp, criterion, optimizer, epoch, args) # TRAIN for one epoch
        acc1, loss = validate(val_loader, val_loader_len, model_gsp.model, criterion, args) # evaluate on VALIDATION set
        ...
        flogger.info(f"Epoch {epoch} time taken for training and validation: {time.time()-t0}")
...
def train(train_loader, train_loader_len, gsp_model, criterion, optimizer, epoch, args):
    ...
    for batch_idx, (images, target) in enumerate(train_loader):
        ...
        if batch_idx % (args.print_freq * 2) == 0:
