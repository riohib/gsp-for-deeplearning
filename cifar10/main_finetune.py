...
def main():
    ...
    if args.resume:
        ...
        checkpoint = torch.load('./checkpoint/gsp_block.pth')
    ...
    setup_pruning_exp_block(model, prune_sps = 0.0)
...
def test(model, criterion, testloader, epoch, args):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/gsp_finetune.pth')
        best_acc = acc
