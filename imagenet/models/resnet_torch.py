...
class ResNet(nn.Module):
    ...
    def sparsity( matrix):
        ...
        device = 'cuda:' + str(matrix.get_device())
    ...
    def get_abs_sps(self):
        nonzero = total = 0
        # print(f"TYPE: {type(model)}")

        for name, param in self.named_parameters():
            print(f" ABS SPS: {name}")
        ...
        print(f"abs sps: {abs_sps} | total {total} |nonz {nonzero}")
        return abs_sps


    def apply_gsp_to_layers(self, name, layer, sps):
        gsp_in_d = {}
    ...
    def _apply_gsp_to_modules(self, name, layer, sps):
        if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
            w_shape = layer.weight.shape
            dim_1 ,  dim_2 = layer.weight.shape[0], layer.weight.shape[1]
    ...
    def _apply_gsp(self) -> None:
        self._apply_gsp_to_modules('conv1', self.conv1, self.sps)

        for name, layer in self.layer1.named_modules():
            if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
                self.apply_gsp_to_layers(name, layer, self.sps)
        ...
        self._apply_gsp_to_modules('fc', self.fc, self.sps)

        print("GSP applied to all layers!")
        if self.logger != None:
            self.logger.info("GSP applied to all layers!")
    
        print(f"MODEL SPS: {self.get_abs_sps()}")
