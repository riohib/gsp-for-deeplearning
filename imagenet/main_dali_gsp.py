...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # args.filelogger.info("Printing DEBUG: 1")
    ...
    if args.pretrained:
        ...
        args.logger.info("Created model from Pytorch Model Zoo!")
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            args.filelogger.info(f"=> loaded checkpoint '{args.resume}' (epoch {checkpoint['epoch']})")
            args.filelogger.info(f"Loaded State Dict: LR: {optimizer.param_groups[0]['lr']:.5f} |" \
    ...
    if args.resume_lr:
        args.filelogger.info(f"Resuming with new Learning Rate! {args.lr}")
        optimizer.param_groups[0]['lr'] = args.lr

    # args.filelogger.info("\n Printing DEBUG: 5 \n")
    args.filelogger.info(f"The sparsity of the model is: {sps_tools.get_abs_sps(model_gsp.model)[0]}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        t1 = time.time() - t0
        args.logger.info(f"Epoch {epoch} took {t1} seconds")
    ...
    args.filelogger.info(f"The sparsity of the model is: {sps_tools.get_abs_sps(model)[0]}")
...
def train(train_loader, train_loader_len, gsp_model, criterion, optimizer, epoch, args):
    ...
    for batch_idx, (images, target) in enumerate(train_loader):
        ...
        if batch_idx % args.print_freq == 0:
            ...
            args.filelogger.info(f"Training: epoch:[{epoch}][{batch_idx}/{train_loader_len}] | Acc@1: {acc1.item():.2f} |" \
...
def validate(val_loader, val_loader_len, model, criterion, args):
    ...
    # Turn off GSP
    # model.module.gsp_training_mode = False
