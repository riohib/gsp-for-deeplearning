...
parser.add_argument( "--exp_name", metavar="EXPNAME", default="baseline",
                    help="Name of the current experiment",)

parser.add_argument( "--logdir", metavar="LOGDIR", default="/logs",
                    help="directory path for log files",)
# parser.add_argument( "--tb_logdir", metavar="TBLOGDIR", default="./results/runs",
#                     help="directory path for log files",)
# parser.add_argument( "--save_dir", metavar="TBLOGDIR", default="./results/runs",
#                     help="directory path for log files",)
...
def setup_experiment(args):
    ...
    logger = Logger(logdir, f'gpu:{args.gpu}')
    args.filelogger = logger.get_file_logger()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        t0 = time.time()
        # train for one epoch
        acc1, loss = train(train_loader, train_loader_len, model, criterion, optimizer, epoch, args)
        t1 = time.time() - t0
        print(f"Epoch {epoch} took {t1} seconds")
        ...
        acc1, loss = validate(val_loader, val_loader_len, model, criterion, args)
    ...
    for batch_idx, (images, target) in enumerate(train_loader):
        ...
        if batch_idx % args.print_freq == 0:
            ...
            args.filelogger.info(f"Training: epoch:[{epoch}][{batch_idx}/{train_loader_len}] | Acc@1: {acc1.item():.2f} |" \
                f"Acc@5: {acc5.item():.2f} | LR: {optimizer.param_groups[0]['lr']:.3f}")
